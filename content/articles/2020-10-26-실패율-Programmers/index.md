...
function solution(N, stages) {
    let personCount = stages.length

    const stageMap = new Map()
    const rateMap = new Map()
    const sortStages = stages.sort((prev, cur) => prev - cur)

    for (const stage of sortStages) {
        const stageCount = stageMap.get(`${stage}`)

        if (stageCount) {
            stageMap.set(`${stage}`, stageCount + 1)
        } else {
    ...
    for (let index = 1; index <= N; index++) {
        const stageClearCount = stageMap.get(`${index}`)

        rateMap.set(
            `${index}`,
            stageClearCount ? stageClearCount / personCount : 0
        )

        if (!isNaN(Number(stageClearCount))) {
            personCount = personCount - stageClearCount
        }
    }

    return [...rateMap]
        .sort(([_, prev], [__, cur]) => cur - prev)
